...
export function NewsCard({
  ...
  return (
    <Link
      href={slug}
      className={`group relative flex flex-col rounded-xs overflow-hidden bg-white shadow-[0_2px_16px_rgba(0,0,0,0.06)] transition-all duration-500 hover:-translate-y-1 w-full ${isFeatured ? "h-full" : "h-auto"
        } ${isHorizontal ? "md:flex-row" : ""
        }`}
    >
      {/* Image container */}
      <div
        className={`relative overflow-hidden bg-[#F5F5F5] shrink-0 w-full ${isFeatured
      ...
      {/* Content */}
      <div
        className={`flex flex-col flex-1 ${isFeatured ? "p-5 md:p-6" : "p-4 md:p-5"
          }`}
      >
        {/* Date */}
        <div className="flex items-center gap-1.5 text-[12px] text-[#9E9E9E] font-body tracking-wide mb-1.5 leading-none">
          <Calendar className="w-[13px] h-[13px] relative -top-[0.5px]" strokeWidth={1.5} />
          <time className="relative top-[0.5px]">{date}</time>
        </div>

        {/* Title */}
        <h3
          className={`font-bold font-display leading-[1.3] text-[#1C1C1C] group-hover:text-primary transition-colors duration-300 ${isFeatured
        ...
        {/* Excerpt */}
        <p
          className={`text-[#4A4A4A] leading-[1.6] font-body mb-4 ${isFeatured
